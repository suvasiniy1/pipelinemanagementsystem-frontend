...
import { CustomActionPosition, ElementType, IControl } from "../models/iControl";
...
const GenerateElements: React.FC<props> = (props) => {
    ...
    const { controlsList, selectedItem, onChange, checked, getListofItemsForDropdown, disable, getCustomElement, ...others } = props;
    ...
    const getElement = (item: IControl) => {
        ...
        switch (item.type) {
            ...
            case ElementType.datepicker:
                return <DATEPICKER item={item} selectedItem={selectedItem} disable={disable} onChange={(e: any) => { onChange && onChange(e, item) }} />
                        ...
                        item.sidebyItem ?
                            <>
                                <div className="form-group row" key={index} hidden={item.hidden}>
                                    ...
                                    <div className="col-6">
                                        <label htmlFor="name" id={`labelFor_${controlsList.find(c=>c.key===item.sidebyItem)}`} className={`col-sm-${controlsList.find(c=>c.key===item.sidebyItem)?.labelSize ? controlsList.find(c=>c.key===item.sidebyItem)?.labelSize : 6} col-form-label ${controlsList.find(c=>c.key===item.sidebyItem)?.isRequired ? "required" : ""}`}>{controlsList.find(c=>c.key===item.sidebyItem)?.key}:</label>
                                    </div>
                                </div>
                                ...
                                <div className="form-group row">
                                    ...
                                    <div className="col-6">
                                        {getElement(controlsList.find(c=>c.key===item.sidebyItem))}
                                    </div>
                                </div>
                            </>
                            :
