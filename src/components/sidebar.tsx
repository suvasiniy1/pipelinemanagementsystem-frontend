    ...
    return (
      ...
      >
        ...
        <Menu style={{ paddingTop: "58px" }}>
          ...
          <SubMenu icon={<MdCampaign color={compaignSubMenu.includes(selectedNavItem) ? activeNavColor : "black"} />} hidden={!Util.isAuthorized("Template")} defaultOpen={compaignSubMenu.includes(selectedNavItem)} title="Campaigns" label="Campaigns">
            <MenuItem hidden={!Util.isAuthorized("Template")} title="Template" component={<Link to="/Template" />} onClick={() => setSelectedNavItem("Template")} icon={<HiTemplate color={selectedNavItem === "Template" ? activeNavColor : "black"} />}>Template</MenuItem>
          ...
          <MenuItem hidden={!Util.isAuthorized("users")} title="Settings" component={<Link to="/users" />} onClick={() => setSelectedNavItem("Settings")} icon={<IoSettings color={selectedNavItem === "Settings" ? activeNavColor : "black"} />}>Manage User</MenuItem>

          <SubMenu icon={<RiAdminFill color={adminSubMenu.includes(selectedNavItem) ? activeNavColor : "black"} />} hidden={!Util.isAuthorized("Admin")} defaultOpen={adminSubMenu.includes(selectedNavItem)} title="Admin" label="Admin">
            <MenuItem hidden={!Util.isAuthorized("Clinic")} title="Clinic" component={<Link to="/Clinic" />} onClick={() => setSelectedNavItem("Clinic")} icon={<FaClinicMedical color={selectedNavItem === "Clinic" ? activeNavColor : "black"} />}>Clinic</MenuItem>
            <MenuItem hidden={!Util.isAuthorized("Source")} title="Source" component={<Link to="/Source" />} onClick={() => setSelectedNavItem("Source")} icon={<BiGitBranch color={selectedNavItem === "Source" ? activeNavColor : "black"} />}>Source</MenuItem>
            <MenuItem hidden={!Util.isAuthorized("Treatment")} title="Treatment" component={<Link to="/Treatment" />} onClick={() => setSelectedNavItem("Treatment")} icon={<FaNotesMedical color={selectedNavItem === "Treatment" ? activeNavColor : "black"} />}>Treatment</MenuItem>
            <MenuItem hidden={!Util.isAuthorized("PipeLineType")} title="Pipeline Type" component={<Link to="/PipeLineType" />} onClick={() => setSelectedNavItem("PipeLineType")} icon={<FaProjectDiagram color={selectedNavItem === "PipeLineType" ? activeNavColor : "black"} />}>Pipeline Type</MenuItem>
          </SubMenu>

          <MenuItem hidden={!Util.isAuthorized("Reporting")} title="Reporting" component={<Link to="/Reporting" />} onClick={() => setSelectedNavItem("Reporting")} icon={<RiDashboard2Fill color={selectedNavItem === "Reporting" ? activeNavColor : "black"} />}>Reporting</MenuItem>
